...
class Scorer(PrepScorer,ScoreProcessor):
	...
	######### FUNCTION HAS BEEN REPLACED BY ITS C++ VERSION ##############
	def calcScores_patristic( self, base_MSA, test_MSA, dist_matrix ):
		...
		for bseq in base_MSA: # loop over all sequences in base MSA
			...
			ti_iter = self.nogapPositions( tseq )
			for bi in self.nogapPositions( bseq ):
				ti = ti_iter.next()
		...
		for ti in range(tc):
			for j in range(n):
				...
				count=0
				for h in test_to_base_map[:,ti]:
					if h==x0:
						scores[j,x0] += dist_matrix[j,count] ## It is compared to itself, but as comparing to itself will yield 0, no need to subtract 1.
					count+=1		
		return scores[ :, 0:bc]

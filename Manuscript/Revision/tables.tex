\documentclass[11pt]{article}

\usepackage[margin=1.0in]{geometry}
%\linespread{1.5}
\usepackage{graphicx}
\usepackage{natbib}
\usepackage{changepage}
\usepackage{rotating}
\pagenumbering{gobble}

\pdfminorversion 4

\bibpunct[,]{(}{)}{;}{a}{}{,}
%\usepackage{tablefootnote}
\usepackage[table]{xcolor}
\usepackage{amsmath}

\renewcommand{\bottomfraction}{.9}
\renewcommand{\topfraction}{.9}
\renewcommand{\textfraction}{0.1}
\renewcommand{\floatpagefraction}{.9}


\usepackage{fancyhdr}

\fancypagestyle{plain}{%
   \fancyhead[R]{\fbox{\big{\textbf{Article; Methods}}}}
   \renewcommand{\headrulewidth}{0pt}
}


\begin{document}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TABLE 1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{table}[htbp]
\begin{adjustwidth}{-1.75cm}{}
%\rowcolors{1}{white}{lightgray}
\caption {\label{tab:summarystats} Summary statistics for effect of masking.}
\begin{tabular}{l l l l l l l}
\hline\noalign{\smallskip}
& & & \multicolumn{4}{c}{True Positive Rates (\% diff from ref aln, for algs)} \\
\cline{4-7}\noalign{\smallskip}
\multicolumn{1}{c}{Selective Profile} & \multicolumn{1}{c}{Number of Taxa} & \multicolumn{1}{c}{Inference Method} & \multicolumn{1}{c}{True} & \multicolumn{1}{c}{Unfiltered} & \multicolumn{1}{c}{Guidance} & \multicolumn{1}{c}{GuidanceP} \\
\noalign{\smallskip}\hline\noalign{\smallskip}
HA  &  11  &  FUBAR  &  0.093  &  0.084  &  0.085 (1.33\%)  &  0.085 (0.74\%) \\
  &    &  PAML  &  0.086  &  0.082  &  0.081 (-0.48\%)  &  0.081 (-0.60\%)\\
\hline
 & 26 & FUBAR & 0.252 & 0.227 & 0.229 (0.84\%) & 0.226 (-0.38\%) \\
 &   & PAML & 0.209 & 0.176 & \textbf{0.178 (1.36\%)$^{\ast\ast}$} & \textbf{0.183 (4.04\%)$^{\ast\ast}$} \\
\hline
 & 60 & FUBAR & 0.551 & 0.474 & 0.479 (0.99\%) & \textbf{0.464 (-2.16\%)$^{\ast}$}  \\
 &  & PAML & 0.422 & 0.347 & 0.342 (-1.68\%) & 0.337 (-2.92\%) \\
 \hline
 & 158 & FUBAR & 0.515 & 0.458 & \textbf{0.467 (1.89\%)$^{\ast}$} & \textbf{0.468 (2.12\%)$^{\ast}$} \\
\hline
GP41  &  11 &  FUBAR  &  0.062  &  0.058  &  0.057 (-1.55\%)  &  0.057 (-1.21\%)\\
  &    &  PAML  &  0.096  &  0.098  &  \textbf{0.095 (-3.49\%)$^{\ast\ast}$}  &  \textbf{0.095 (-3.80\%)$^{\ast\ast}$} \\
\hline
 & 26 & FUBAR & 0.216 & 0.196 & \textbf{0.20 (1.89\%)$^{\ast}$} & 0.197 (0.36\%) \\
 & & PAML & 0.237 & 0.216 & 0.220 (1.54\%) & 0.217 (0.244\%) \\
 \hline
 & 60 & FUBAR & 0.359 & 0.308 & \textbf{0.313 (1.77\%)$^{\ast}$} & 0.304 (-1.16\%)\\
 & & PAML & 0.341 & 0.304 & 0.302 (-0.77\%) & \textbf{0.296 (-2.71\%)$^{\ast\ast}$} \\
 \hline
 & 158 & FUBAR & 0.348 & 0.320 & \textbf{0.325 (1.77\%)$^{\ast\ast}$} & \textbf{0.326 (2.02\%)$^{\ast\ast}$} \\
 \hline
\end{tabular}
\newline
\textsc{note.}--- Significance levels:  $^{\ast\ast} P < 0.001}$; $^{\ast} P < 0.01$. Each column displays the mean TPR and for algorithms the percent change from unfiltered. bold are significant changes from unfiltered. All significance levels were corrected for multiple comparisons using the R multcomp package \citep{Hothorn2008}.
\end{adjustwidth}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TABLE 2 (???) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\begin{table}[htbp]
\begin{adjustwidth}{-1.75cm}{}
\caption {\label{tab:omegastats} MLE and fubar mean dN affected by filtering.}
\begin{tabular}{l l l c c}
\hline\noalign{\smallskip}
& & & \multicolumn{2}{c}{Difference in Omega Value (filtered - unfiltered)} \\
\cline{4-5}\noalign{\smallskip}
\multicolumn{1}{c}{Selective Profile} & \multicolumn{1}{c}{Number of Taxa} & \multicolumn{1}{c}{Inference Method} & \multicolumn{1}{c}{Guidance} & \multicolumn{1}{c}{GuidanceP} \\
\noalign{\smallskip}\hline\noalign{\smallskip}
HA & 26 & PAML &  0.0023 & \textbf{0.0072$^{\ast\ast\ast}$}  \\
 & 60 & FUBAR & 0.0056 & \textbf{-0.0083$^{\ast}$}\\
 & 158 & FUBAR & 0.0058 & 0.0026\\
\hline
GP41 & 11 & PAML & \textbf{-0.0033$^{\ast\ast}$} & \textbf{-0.0036$^{\ast\ast}$} \\ 
 & 26 & FUBAR & \textbf{0.0037$^{\ast\ast}$} & 0.00074 \\ 
 & 26 & PAML & \textbf{0.0046$^{\ast\ast}$} & 0.00096 \\ 
 & 60 & FUBAR & \textbf{0.0054$^{\ast\ast}$} & -0.0032 \\ 
 & 60 & PAML &  -0.00024 & \textbf{-0.0069$^{\ast}$}\\  
 & 158 & FUBAR & \textbf{0.0047$^{\ast}$} & 0.0044\\
\end{tabular}
\newline
\textsc{note.}--- Simulation schemes not shown in table were all NS. Info here pertains only to those cases in which significant differences between unfiltered and a Guidance-based were detected. Significance levels: $^{\ast\ast\ast} P < 0.001}$; $^{\ast\ast} P < 0.01$; $^{\ast} P < 0.05$. All significance levels were corrected for multiple comparisons using the R multcomp package \citep{Hothorn2008}.
\end{adjustwidth}
\end{table}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TABLE S1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\noindent Table S1. Full version of Table 1 in main body of MS.
\begin{sidewaystable}[htbp]
\begin{adjustwidth}{-2.25cm}{}
\begin{tabular}{l l l l l l l l l l l}
\hline\noalign{\smallskip}
& & & \multicolumn{8}{c}{True Positive Rates (\% diff from ref aln, for algs)} \\
\cline{4-11}\noalign{\smallskip}
\multicolumn{1}{c}{Profile} & \multicolumn{1}{c}{Num} & \multicolumn{1}{c}{Inf Method} & \multicolumn{1}{l}{True} & \multicolumn{1}{l}{Unfiltered} & \multicolumn{1}{l}{Guidance} & \multicolumn{1}{l}{BMweights} & \multicolumn{1}{l}{PDweights} & \multicolumn{1}{l}{GuidanceP} & \multicolumn{1}{l}{BMweightsP} & \multicolumn{1}{l}{PDweightsP} \\
\noalign{\smallskip}\hline\noalign{\smallskip}
HA  &  11  &  FUBAR  &  0.093  &  0.084  &  0.085 (1.33\%)  &  0.086 (1.57\%)  &  0.085 (0.62\%)  &  0.085 (0.74\%)  &  0.086 (1.57\%)  &  0.086 (1.92\%)  \\
  &    &  PAML       &  0.086  &  0.082  &  0.081 (-0.48\%)  &  0.081 (-0.48\%)  &  0.082 (0.62\%)  &  0.081 (-0.60\%)  &  0.081 (-0.97\%)  &  0.081 (-0.97\%)  \\
\hline
  &  26  &  FUBAR    &  0.252  &  0.227  &  0.229 (0.84\%)  &  0.228 (0.48\%)  &  0.228 (0.31\%)  &  0.226 (-0.4\%)  &  0.226 (-0.4\%)  &  0.227 (-0.09\%)  \\
  &    &  PAML       &  0.209  &  0.176  &  0.178 (1.36\%)  & 0.178 (1.36\%)  &  0.178 (1.47\%)  &  \textbf{0.183 (4.04\%)}$^{\ast\ast\ast}$  &  \textbf{0.183 (4.04\%)}$^{\ast\ast\ast}$  &  \textbf{0.182 (3.81\%)}$^{\ast\ast\ast}$  \\
\hline
  &  60  &  FUBAR    &  0.551  &  0.474  &  0.479 (0.99\%)  &  0.476 (0.48\%)  &  0.478 (0.90\%)  &  \textbf{0.464 (-2.16\%)}$^{\ast\ast}$  &  \textbf{0.464 (-2.13\%)}$^{\ast\ast}$  &  \textbf{0.463 (-2.43\%)}$^{\ast\ast\ast}$  \\
  &    &  PAML       &  0.422  &  0.347  &  0.342 (-1.68\%)  &  0.345 (-0.62\%)  &  0.341 (-1.94\%)  &  0.337 (-2.92\%)  &  0.333 (-4.22\%)  &  0.337 (-3.06\%)  \\
\hline
  &  158  &  FUBAR   &  0.515  &  0.458  & \textbf{0.467 (1.88\%)}$^{\ast\ast}$  &  \textbf{0.4667 (1.88\%)}$^{\ast\ast}$  &  \textbf{0.4671 (1.97\%)}$^{\ast\ast}$  &  \textbf{0.468 (2.12\%)}$^{\ast\ast}$  &  \textbf{0.467 (2.03\%)}$^{\ast\ast}$  &  \textbf{0.467 (1.93\%)}$^{\ast\ast}$  \\
\hline
GP41  &  11 &  FUBAR  &  0.062  &  0.058  &  0.057 (-1.55\%)  &  0.057 (-1.55\%)  &  0.058 (-0.69\%)  &  0.057 (-1.21\%)  &  0.057 (-1.03\%)  &  0.057 (-1.38\%)  \\
  &    &  PAML        &  0.0959  &  0.0982  &  \textbf{0.0948 (-3.49\%)}$^{\ast\ast\ast}$  &  \textbf{0.095 (-3.39\%)}$^{\ast\ast\ast}$  &  \textbf{0.095 (-3.49\%)}$^{\ast\ast\ast}$  &  \textbf{0.095 (-3.80\%)}$^{\ast\ast\ast}$  &  \textbf{0.094 (-3.90\%)}$^{\ast\ast\ast}$  &  \textbf{0.094 (-4.31\%)}$^{\ast\ast\ast}$  \\
\hline
  &  26  &  FUBAR     &  0.216  &  0.196  &  \textbf{0.200 (1.89\%)}$^{\ast\ast}$  &  \textbf{0.200 (2.04\%)}$^{\ast\ast}$  &  \textbf{0.199 (1.48\%)}$^{\ast}$  &  0.197 (0.36\%)  &  0.197 (0.36\%)  &  0.196 (-0.10\%)  \\ 
  &    &  PAML        &  0.237  &  0.216  &  0.220 (1.53\%)  &  \textbf{0.220 (1.77\%)}$^{\ast}$  &  0.219 (1.44\%)  &  0.217 (0.24\%)  &  0.217 (0.15\%)  &  0.215 (-0.78\%)  \\ 
\hline
  &  60  &  FUBAR     &  0.359  &  0.308  &  \textbf{0.313 (1.77\%)}$^{\ast\ast}$  &  \textbf{0.313 (1.83\%)}$^{\ast\ast}$  &  \textbf{0.312 (1.44\%)}$^{\ast}$  &  0.304 (-1.16\%)  &  0.304 (-1.13\%)  &  \textbf{0.301 (-2.07\%)}$^{\ast\ast\ast}$  \\
  &    &  PAML        &  0.341  &  0.304  &  0.302 (-0.77\%)  &  0.303 (-0.14\%)  &  0.300 (-1.13\%)  &  \textbf{0.296 (-2.71\%)}$^{\ast\ast\ast}$  &  \textbf{0.296 (-2.61\%)}$^{\ast\ast}$  &  \textbf{0.293 (-3.53\%)}$^{\ast\ast\ast}$  \\
\hline
  &  158  &  FUBAR    &  0.348  &  0.320  &  \textbf{0.325 (1.77\%)}$^{\ast\ast\ast}$  &  \textbf{0.324 (1.37\%)}$^{\ast\ast\ast}$  &  \textbf{0.326 (1.93\%)}$^{\ast\ast\ast}$  &  \textbf{0.326 (2.02\%)}$^{\ast\ast\ast}$  &  \textbf{0.326 (1.93\%)}$^{\ast\ast\ast}$  &  \textbf{0.325 (1.77\%)}$^{\ast\ast\ast}$  \\
\hline
\end{tabular}
\newline
\textsc{note.}--- %Significance levels: $^{\ast\ast\ast} = P < 0.001$; $^{\ast\ast} = P < 0.01$; $^{\ast} = P < 0.05$. Sig level = is sig diff from refaln? Each column displays the mean TPR w/ difference from refaln in paren.. All significance levels were corrected for multiple comparisons using the R multcomp package \citep{Hothorn2008}.
\end{adjustwidth}
\end{sidewaystable}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TABLE S2 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\noindent Table S2. Average percent residues masked (average absolute number residues masked) for algorithms, using masking threshold of 0.5.
\begin{sidewaystable}[htbp]
\begin{adjustwidth}{-2.25cm}{}
%\rowcolors{1}{white}{lightgray}
\begin{tabular}{l l l l l l l l l}
\hline\noalign{\smallskip}
& & & \multicolumn{6}{c}{Percent Residues Masked (absolute nummasked)} \\
\cline{4-9}\noalign{\smallskip}
\multicolumn{1}{c}{Profile} & \multicolumn{1}{c}{Num} & \multicolumn{1}{c}{Inf Method} & \multicolumn{1}{c}{Guidance} & \multicolumn{1}{c}{GuidanceP} & \multicolumn{1}{c}{BMweights} & \multicolumn{1}{c}{BMweightsP} & \multicolumn{1}{c}{PDweights} & \multicolumn{1}{c}{PDweightsP} \\
\noalign{\smallskip}\hline\noalign{\smallskip}
HA  &  11  &  FUBAR  &  0.01\% (43.25)  &  0.01\% (42.49)  &  0.01\% (44.35)  &  0.02\% (89.32)  &  0.02\% (87.69)  &  0.026\% (113.67)  \\
  &    &  PAML  &  0.01\% (43.25)  &  0.01\% (42.49)  &  0.01\% (44.35)  &  0.02\% (89.32)  &  0.02\% (87.69)  &  0.026\% (113.67)  \\
 \hline
  &  26  &  FUBAR  &  0.01\% (107.75)  &  0.01\% (106.73)  &  0.01\% (108.67)  &  0.025\% (256.84)  &  0.024\% (251.01)  &  0.026\% (264.28)  \\
  &    &  PAML  &  0.01\% (107.75)  &  0.01\% (106.73)  &  0.01\% (108.67)  &  0.025\% (256.84)  &  0.024\% (251.01)  &  0.026\% (264.28)  \\
  \hline
   &  60  &  FUBAR  &  0.027\% (653.38)  &  0.026\% (634.76)  &  0.032\% (762.93)  &  0.094\% (2280.15)  &  0.096\% (2326.67)  &  0.104\% (2522.89)  \\
  &    &  PAML  &  0.027\% (653.38)  &  0.026\% (634.76)  &  0.032\% (762.93)  &  0.094\% (2280.15)  &  0.096\% (2326.67)  &  0.104\% (2522.89)  \\
 \hline
   &  158  &  FUBAR  &  0.006\% (382.88)  &  0.006\% (366.12)  &  0.006\% (413.73)  &  0.018\% (1126.08)  &  0.019\% (1198.67)  &  0.019\% (1245.29)  \\
 \hline
 GP41  &  11  &  FUBAR  &  0.009\% (40.56)  &  0.009\% (40.01)  &  0.009\% (41.82)  &  0.021\% (93.1)  &  0.021\% (91.96)  &  0.027\% (117.72)  \\
  &    &  PAML  &  0.009\% (40.56)  &  0.009\% (40.01)  &  0.009\% (41.82)  &  0.021\% (93.1)  &  0.021\% (91.96)  &  0.027\% (117.72)  \\
  \hline
   &  26  &  PAML  &  0.014\% (143.81)  &  0.014\% (141.86)  &  0.014\% (148.7)  &  0.042\% (435.8)  &  0.044\% (461.09)  &  0.049\% (512.64)  \\
  &    &  FUBAR  &  0.014\% (143.81)  &  0.014\% (141.86)  &  0.014\% (148.7)  &  0.042\% (435.8)  &  0.044\% (461.09)  &  0.049\% (512.64)  \\
  \hline
   &  60  &  FUBAR  &  0.027\% (655.49)  &  0.027\% (639.34)  &  0.032\% (777.8)  &  0.091\% (2169.87)  &  0.094\% (2253.62)  &  0.102\% (2447.19)  \\
  &    &  PAML  &  0.027\% (655.49)  &  0.027\% (639.34)  &  0.032\% (777.8)  &  0.091\% (2169.87)  &  0.094\% (2253.62)  &  0.102\% (2447.19)  \\
 \hline
   &  158  &  FUBAR  &  0.006\% (363.15)  &  0.006\% (350.34)  &  0.006\% (396.34)  &  0.017\% (1060.16)  &  0.016\% (1036.91)  &  0.017\% (1103.85)  \\
\hline
\end{tabular}
\newline
\textsc{note.}--- %Significance levels: $^{\ast\ast} P < 0.001}$; $^{\ast} P < 0.01$. Sig level = is sig diff from refaln? Each column displays the mean (perc_masked) TPR. All significance levels were corrected for multiple comparisons using the R multcomp package \citep{Hothorn2008}.
\end{adjustwidth}
\end{sidewaystable}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% TABLE S3 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{table}
\begin{adjustwidth}{-1.5cm}{}
 \caption {\label{tab:cutoffs} Effect of masking cutoff on TPR of positive-selection inference.}
\begin{tabular}{l l c c c c c c c}
\hline\noalign{\smallskip}
& & \multicolumn{3}{c}{Guidance} & & \multicolumn{3}{c}{GuidanceP}   \\
\cline{3-5} \cline{7-9}\noalign{\smallskip}
\multicolumn{1}{c}{Num} & \multicolumn{1}{c}{Inf} & \multicolumn{1}{c}{0.5 TPR} & \multicolumn{1}{c}{0.9 TPR} & \multicolumn{1}{c}{Percent TPR Change} & & \multicolumn{1}{c}{0.5 TPR} & \multicolumn{1}{c}{0.9 TPR} & \multicolumn{1}{c}{Percent TPR Change}  \\
\hline\noalign{\smallskip}
11  &  FUBAR  &  0.0854  &  0.0818  &  \textbf{-4.22\%$^{\ast}}$  & &  0.0849  &  0.0793  &  \textbf{-6.60\%$^{\ast\ast}$}  \\
  &  PAML  &  0.0812  &  0.0791  &  -2.59\% & &  0.0811  &  0.0743  &  \textbf{-8.38\%$^{\ast\ast}$}   \\
\hline
26  &  FUBAR  &  0.229  &  0.227  &  -0.87\% & &  0.226  &  0.215  &  \textbf{-5.21\%$^{\ast\ast\ast}$}  \\
  &  PAML  &  0.178  &  0.179  &  0.67\% & &  0.183  &  0.175  &  \textbf{-4.33\%$^{\ast\ast}$}  \\
\hline
60  &  FUBAR  &  0.479  &  0.458  &  \textbf{-4.43$^{\ast\ast\ast}$} & &  0.464  &  0.332  &  \textbf{-28.5\%$^{\ast\ast}$}  \\
  &  PAML  &  0.342  &  0.327  &  -4.19\%  & &  0.337  &  0.256  &  \textbf{-24.1\%$^{\ast\ast}$}  \\
\hline
158  &  FUBAR  &  0.467  &  0.463  &  -0.73\%  & & 0.468  &  0.452  &  \textbf{-3.44\%$^{\ast\ast\ast}$}  \\
\hline
\end{tabular}
\newline
\textsc{note.}--- Significance levels: $^{\ast\ast\ast} P < 10^{-4}$; $^{\ast\ast} P < 10^{-3}$; $^{\ast} P < 10^{-2}$. 
0.5 TPR: average TPR for alignments masked at a cutoff of 0.5; 0.9 TPR: average TPR for alignments masked at a cutoff of 0.9; Percent TPR Decrease: average percent decrease in TPR recovered between alignments masked at cutoffs of 0.5 and 0.9. All significance levels were corrected for multiple comparisons using the R multcomp package \citep{Hothorn2008}.
\end{adjustwidth}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%





\end{document}
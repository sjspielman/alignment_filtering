...
The primary goal of MSA filtering is to remove excessive noise while preserving informative data. Unfortunately, we recovered few conditions for which filtering consistently removed noise but preserved signal. While Guidance-based filtering was useful for FPR levels ranging from around $1\% - 4\%,$ this range was extremely narrow, and it is impossible to know if a given real data set will actually fall in this range. Moreover, that the more statistically-controlled phylogenetically-corrected did not improve upon the original Guidance algorithm indicated the minimal benefits that Guidance-based filtering produced in the first place. As the original Guidance did not prove to be a robust method, it was unsurprising that the phylogenetically-corrected scoring algorithms we implemented did not affect results.

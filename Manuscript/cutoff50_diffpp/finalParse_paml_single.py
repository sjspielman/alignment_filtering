...
def getConsensus(allMaps):
	...
	return (mapRef, mapTrue)
################################################################################################
def parsePAML(paml_file, alnlen, map):
	allpaml=[]
	testprobs=[]
...
genes={'or5':0.518, 'rho':0.3025, 'prk':0.0625} ## these are the paml pp cutoffs which yield a fpr=1%. note that there's no flat because paml takes forever.
masks={'30':'thirty','50':'fifty', '70':'seventy', '90':'ninety'}
base='seqs_real'
method='pamlM8'
mask='50'

outfile='analysis/pamlsingle_mask50.txt'
...
for gene in genes:
	...
	pamldir = method+'/paml_linsi_'+gene+'_'+base+'/'
	...
	for n in range(100):
		...
		refpaml=pamldir+'refaln'+str(n)+'.fasta.rst'
		...
		for case in prefs:
			if case == 'refaln':
				parsed=refparsed
				testprobs = parsePAML(refpaml, alnlen, mapRef)	
				
			if case == 'truealn':
				## likely these lines are wrong! fix them if you ever do this.
				parsed=trueparsed
				testprobs = parsePAML(mapTrue, alnlen, truefu)
			...
			outhandle.write(str(n)+'\t'+str(tprate)+'\t'+str(fprate)+'\t'+str(tnrate)+'\t'+str(fnrate)+'\t'+str(accuracy)+'\t'+str(taxamasked)+'\t'+str(percentmasked)+'\t'+str(percentgaps)+'\t'+case+'\t'+gene+'\tlinsi\t'+method+'\tzero\n')	
		...
		for case in prefix:
			...
			casealn=alndir+name
			casepaml=pamldir+name+'.rst'
			parsed=AlignIO.read(casealn, 'fasta')
			
			testprobs = parsePAML(casepaml, alnlen, mapRef)	
			...
			(tp,tn,fp,fn,tprate,fprate,tnrate,fnrate,accuracy)=sweepRates(float(genes[gene]), truepos, testprobs)
			(taxamasked, percentmasked, percentgaps)=percentMaskedRes(parsed, alnlen, numseq)
			outhandle.write(str(n)+'\t'+str(tprate)+'\t'+str(fprate)+'\t'+str(tnrate)+'\t'+str(fnrate)+'\t'+str(accuracy)+'\t'+str(taxamasked)+'\t'+str(percentmasked)+'\t'+str(percentgaps)+'\t'+case+'\t'+gene+'\tlinsi\t'+method+'\t'+masks[mask]+'\n')	
outhandle.close()

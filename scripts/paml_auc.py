...
for gene in genes:
	...
	pamldir = 'pamlM8/paml_linsi_'+gene+'_seqs_real/'
	...
	for n in range(100):
		...
		outhandle.write(str(n)+'\t'+str(auc)+'\trefaln\t'+gene+'\tpaml\tzero\n')	
		...
		for case in prefix:
			...
			auc=calcAUC(truepos, testprobs, cutoffs)	
			outhandle.write(str(n)+'\t'+str(auc)+'\t'+case+'\t'+gene+'\tpaml\t'+penal+'\n')	
outhandle.close()

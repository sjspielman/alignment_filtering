### 6/12/13
# Masking 70,90 (residues and columns). Only the guidance+gweights algorithms.
# Uses mafft (simplest) for all things.

## 6/17/13 - MANY OF THE ALIGNMENTS DIDN'T GO THROUGH?!?! REDOING THE WEIRD ONES HERE.
...
## 12 directories
dir=['', 'bigindel_64rc', 'medindel_64rc', 'shortindel_64rc', 'bigindel_64p', 'medindel_64p', 'shortindel_64p', 'bigindel_16r', 'medindel_16r', 'shortindel_16r', 'bigindel_64r', 'medindel_64r', 'shortindel_64r']
...
for blah in range(100):
	...
	BootDir='BootDir/'
	#Guidance modules
	amod = MAFFTAligner("/home/sjs3495/bin/mafft", "--thread 1 --quiet --retree 1" )
	...
	#final_r=residues are masked. finalc=columns are masked, and column score is obtained through a REGULAR average. (weighted average no more than 1% difference so not done)
...
for x in range(100):
	for case in prefix:
		...
		size_alnfile=os.path.getsize(alnfile)
		if size_alnfile==0:
			print "ALIGN FAILED", alnfile
			stop=1
			break
		...
		while size==0:
			...
			size=os.path.getsize(newfile)
	if stop==1:
		print "stopping treeing due to missing file", alnfile
		break
